...
@Composable
fun LoginScreen (modifier: Modifier = Modifier,navController: NavController,AuthViewModule: AuthViewModule=viewModel()) {
    ...
    Column (modifier = modifier.fillMaxSize().padding(32.dp),
        ...
        Button(onClick = {
            isLoading = true
            AuthViewModule.login(email, password ){
            ...
            modifier = Modifier.fillMaxWidth()
                .height(60.dp)) {
            Text( text = if (isLoading) "Loading..." else "login",fontSize = 25.sp)
        }
    }
}